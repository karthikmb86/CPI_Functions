print('INFO - Current And Last Successful Load Dates Start ===========================================================================');
        ...
        # MRP Controller 1
        if ($G_MRP_CONTROLLER1 is null or $G_MRP_CONTROLLER1 = 'L01')
        print('INFO - MRP Controller 1 variable $G_MRP_CONTROLLER1 with default value: '|| $G_MRP_CONTROLLER1);
        else
        begin
        print('INFO -  MRP Controller 1 area variable $G_MRP_CONTROLLER1 set by input to: '|| $G_MRP_CONTROLLER1);
        $G_STANDARD_LOAD = 'no';
        end

        # MRP Controller 2
        if ($G_MRP_CONTROLLER2 is null or $G_MRP_CONTROLLER2 = 'M01')
        print('INFO - MRP Controller 2 variable $G_MRP_CONTROLLER2 with default value: '|| $G_MRP_CONTROLLER2);
        else
        begin
        print('INFO -  MRP Controller 2 area variable $G_MRP_CONTROLLER2 set by input to: '|| $G_MRP_CONTROLLER2);
        $G_STANDARD_LOAD = 'no';
        end

        # MRP Controller 3
        if ($G_MRP_CONTROLLER3 is null or $G_MRP_CONTROLLER3 = 'P01')
        print('INFO - MRP Controller 3 variable $G_MRP_CONTROLLER3 with default value: '|| $G_MRP_CONTROLLER3);
        else
        begin
        print('INFO -  MRP Controller 3 area variable $G_MRP_CONTROLLER3 set by input to: '|| $G_MRP_CONTROLLER3);
        $G_STANDARD_LOAD = 'no';
        end

        # Past Days - Data will be pulled starting from these past n days onwards thereby reducing the query result size
        if ($G_PAST_DAYS is null or $G_PAST_DAYS = 30)
        print('INFO - Past number of days variable $G_PAST_DAYS with default value: '|| $G_PAST_DAYS);
        else
        begin
        print('INFO -  Past number of days variable $G_PAST_DAYS set by input to: '|| $G_PAST_DAYS);
        $G_STANDARD_LOAD = 'no';
        end
